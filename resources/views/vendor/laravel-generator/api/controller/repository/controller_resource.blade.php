...
@else
        return $this->sendResponse(new {{ $config->modelNames->name }}Resource(${{ $config->modelNames->camel }}), '{{ $config->modelNames->human }} se guardó correctamente');
@endif
    }
...
@else
        return $this->sendSuccess('{{ $config->modelNames->human }} se eliminó correctamente');
@endif
    }
}
